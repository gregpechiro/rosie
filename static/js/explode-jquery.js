...
$(document).ready(function() {
	...
	if(cssPrefix) {
		// 300 x 233
		var cols = 50; // Desired columns
		var rows = 50; // Desired rows
		...
		for(x = 0; x < rows; x++) {
			...
			for(y = 0; y < cols; y++) {
				...
				last = jQuery("<span />").attr("style","width:" + (singleWidth) +
                "px;height:" + (singleHeight) + "px;background-position:-" +
                (singleWidth * y) + "px -" + (singleHeight * x) + "px;-" +
                cssPrefix + "-transition-property: -" + cssPrefix + "-transform; -" +
                cssPrefix + "-transition-duration: 200ms; -" + cssPrefix +
                "-transition-timing-function: ease-out; -" + cssPrefix +
